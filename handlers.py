...
async def api_register_user(*,email,name,passwd):
	...
	if len(users) > 0:
		raise APIValueError('注册：失败','邮箱','该邮箱已经在使用中')
	uid = next_id()
	sha1_passwd = '%s:%s' %(uid,passwd)
	user = User(id=uid,name=name.strip(),email=email,passwd=hashlib.sha1(sha1_passwd.encode('utf-8')).hexdigest(),image='http://www.gravatar.com/avatar/%s?dd=mm&s=120' %hashlib.md5(email.encode('utf-8')).hexdigest())
	await user.save()
	#制作会话cookie
	r = web.Response()
	r.set_cookie(COOKIE_NAME,user2cookie(user,86400),max_age=86400,httponly=True)

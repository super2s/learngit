...
def init_jinja2(app,**kw):
	...
	fileters = kw.get('filters',None)
	if filters is not None:
		for name, f in filter.items():
			env.filters[name]=file
		app['__templating__'] = env
async def logger_facotory(app,handler):#记录器
...
async def response_factory(app,handler):
	async def response(request):
		...
		if isinstance(r,dict):
			...
			if template is None:
				resp = web.Response(body = json.dumps(r,ensure_ascii=False,deault=lambda o:o.__dict__).encode('utf-8'))
